...
def get_templates_dir(toolkit_root: Path) -> Path:
    """Get the path to migration_prompts directory."""
    return toolkit_root / "migration_prompts"


def load_template(toolkit_root: Path, template_name: str) -> str:
    """Load a template file from migration_prompts directory.
...
def select_template(
    ...
    """Select appropriate template based on diff type.
    
    Priority:
    1. ChangelogEntry.migration.prompt (if specified)
    2. Paradigm shift detection
    3. StepDiff-based selection
    4. FieldDiff-based selection
    ...
    # Priority 2: Paradigm shift
    if paradigm and paradigm.detected:
        if paradigm.prompt_template:
            return paradigm.prompt_template
        # Source is markdown → JSON conversion
        if paradigm.source_file and str(paradigm.source_file).endswith(".md"):
            return "template_prose_to_json.md"
        # JSON → markdown (rare)
        if paradigm.target_file and str(paradigm.target_file).endswith(".md"):
            return "template_json_to_prose.md"
    
    # Priority 3: Step-level
    if step_diff:
        status = step_diff.status.upper() if step_diff.status else ""
        if status == "MISSING":
            if step_diff.paradigm_shift_from:
                return "template_prose_to_json.md"
            return "template_add_step.md"
        if status == "RENAME":
            return "template_step_rename.md"
        if status == "UNKNOWN":
            # Unknown step (project extension)
            return "template_archive_extension.md"
    
    # Priority 4: Field-level
    if field_diff:
        if field_diff.diff_type == DiffType.MISSING_REQUIRED:
            # If we have context to infer from, use infer_missing
            # Otherwise use add_field for direct addition
            return "template_add_field.md"
        if field_diff.diff_type == DiffType.EXTRA_FIELD:
            return "template_remove_field.md"
        if field_diff.diff_type == DiffType.RENAME_CANDIDATE:
            return "template_rename_field.md"
        if field_diff.diff_type == DiffType.TYPE_MISMATCH:
            return "template_type_coercion.md"
        if field_diff.diff_type == DiffType.SCHEMA_REF_OUTDATED:
            # Broken references need trace validation
            return "template_validate_traces.md"
    
    # Default fallback
    return "template_resolve_conflict.md"
