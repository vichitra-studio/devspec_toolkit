...
| `digest_validator` | `tools/specdev_tools/{validation,validators,canonical,analysis,registry,generation,core}/**/*.py` | AST (Python `ast` module) | `module_path`, exported check functions, error codes (E-codes) and warning codes (W-codes) emitted, schema paths referenced, guide refs. |
