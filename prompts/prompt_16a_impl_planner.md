...
*   `checklist`: A list of **Atomic Requirements**.
    ...
    *   `spec_ref`: **Structured Object**. `{ type, id, line_range, commit_hash }`.
        ...
        *   *Rule*: `type` must be a value from `$defs.specRef.type` enum in `schema/16_impl_context.schema.json` (the schema is the authority). The full enum is: `"fr" | "api" | "nfr" | "inv" | "fixture" | "doc" | "code" | "task"`. Decision rules: use `"task"` when `id` equals a `task-*` task_id from Step 14 (roadmap coverage items) — do NOT mislabel as `"fr"`; use `"doc"` when pointing to a specification document, schema, or guide (e.g., a `vc:` schema URI); use `"code"` when pointing to a specific function, class, or code block in an implementation file; use `"fr" | "api" | "nfr" | "inv" | "fixture"` only when `id` matches the corresponding kebab-case ID in the relevant spec artifact.
