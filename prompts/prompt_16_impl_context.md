...
The anchor must:
1. Declare the full scope spanning every non-`done` milestone in this cycle — `pending`, `in_progress`, and `deferred` per the shared `milestoneStatus` enum (`plan.summary`).
...
2. **Scope**: Derive `plan.summary.functional_summary`, `scope_in`, `scope_out` spanning **every milestone in this cycle regardless of `status`** (`pending`/`in_progress`/`done`/`deferred` per the shared `milestoneStatus` enum). The anchor is the union of milestone scope — not a milestone plan itself.
3. **Milestone Index**: For every milestone in `14_roadmap.json` that participates in this cycle (any `status` value: `pending`, `in_progress`, `done`, or `deferred`), emit one `plan.milestone_index[]` entry with `milestone_id`, `context_path` (pointing at the 16a plan file), `status` (the same enum value), `fr_refs`, `checklist_id_prefix` (SCREAMING_SNAKE namespace the 16a plan will use for its checklist IDs), and a one-line `summary`. Drop a milestone from the index only when it has been removed from the roadmap entirely.
...
- Every milestone in `14_roadmap.json` (whatever its `status`: `pending`, `in_progress`, `done`, or `deferred`) appears in `plan.milestone_index`, OR is explicitly listed in `scope_out` with a rationale carried in `plan.ambiguities`. The anchor is the cumulative ledger across cycles, not a snapshot of "what's in flight right now" — `done` milestones must remain indexed so checklist_id_prefix collisions and traceability stay enforceable.
...
*   **Mis-schemaed milestone (W589)**: A file in `impl_context/` declares a `$schema` other than `vc:16-impl-context` (common when a pre-split anchor was moved into `impl_context/` without updating the header). *Fix*: Either set `$schema: vc:16-impl-context` on the file so the anchor can read it, or delete the file if it is no longer a milestone plan. The migration template at `prompts/migration/template_impl_context.md` walks through the post-split layout.
*   **Context path missing on disk (W607)**: A `milestone_index[].context_path` declaration points at a file that does not exist under `impl_context/` (typo, un-committed rename, or file deleted without index update). *Fix*: Either create the milestone plan at the declared path, or correct the `context_path` entry in the anchor to match the real filename. Silent drop would remove the milestone from E308/E309 comparison — W607 surfaces the mismatch at author time.
*   **Legacy anchor schema (W608)**: The artifact at `spec/16_impl_context.json` still declares `$schema: vc:16-impl-context` (the pre-0.6.0 milestone-plan shape) instead of `vc:16-anchor`. Schema validation against the registered milestone-plan schema still passes, but every anchor-only drift check (E308/E309/W587) silently no-ops on the legacy shape — there is no migration forcing function without W608. *Fix*: Follow the breaking-change migration in `changelog/unreleased.md` — move per-milestone checklist content into `spec/impl_context/<milestone>_plan.json`, rewrite the anchor file against `vc:16-anchor` with `plan.summary/ambiguities/drift/milestone_index`, and set `artifact_role: "anchor"`.
*   **Misfiled anchor (W609)**: An artifact with `artifact_role: "anchor"` landed inside `spec/impl_context/` instead of at `spec/`. The anchor validator runs (routing correctly demotes the dispatch), but its `impl_context` sibling directory resolves to `spec/impl_context/impl_context/` — which never exists — so every drift check silently no-ops. *Fix*: Move the file to `spec/16_impl_context.json`.
