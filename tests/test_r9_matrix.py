...
class TestCheckCoverageThresholds(unittest.TestCase):
    ...
    def test_below_threshold_warn_mode_emits_w592(self):
        ...
        with tempfile.TemporaryDirectory() as repo_root:
            ...
            self.assertIn("W592", errors[0])
            self.assertIn("COVERAGE_THRESHOLD_WARN", errors[0])
            self.assertIn("fr_coverage", errors[0])
            self.assertIn("50.0%", errors[0])
    ...
    def test_below_threshold_error_mode_emits_e592(self):
        ...
        with tempfile.TemporaryDirectory() as repo_root:
            ...
            errors = _check_coverage_thresholds(coverage, repo_root)
            self.assertEqual(len(errors), 1)
            self.assertIn("E592", errors[0])
            self.assertIn("COVERAGE_THRESHOLD_BREACH", errors[0])
            self.assertIn("fr_coverage", errors[0])
    ...
    def test_default_mode_is_warn(self):
        ...
        with tempfile.TemporaryDirectory() as repo_root:
            ...
            self.assertEqual(len(errors), 1)
            self.assertIn("W592", errors[0])
            self.assertNotIn("E592", errors[0])
