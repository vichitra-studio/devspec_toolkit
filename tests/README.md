...
- [`expectations/trace_matrix.json`](expectations/trace_matrix.json) captures expected FR→API→Fixture→NFR wiring for quick drift detection.

## Guardrails

- Filenames are **kebab/underscore**; IDs remain **kebab-case** inside JSON.
- No schema redefinition. All validation relies on `$schema` in spec artifacts and the registry in [tools/](../tools/).
...
- Run the [core validation commands](../docs/developers/reference.md#core-validation-commands), then compare `tools/trace_matrix.json` against [`tests/expectations/trace_matrix.json`](expectations/trace_matrix.json) (e.g., `diff -u ... || true`; sample output lives in [example/tools/trace_matrix.json](../example/tools/trace_matrix.json)).
