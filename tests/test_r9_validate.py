...
The promotion logic (validate.py lines 261–287) uses PROMOTABLE_PAIRS from
errors.py to dynamically promote warning codes to error codes based on
...
def _run_validate_dir_with_injected_failures(
    root: Path, injected_failures: list[str], env_overrides: dict[str, str] | None = None
) -> list[str]:
...
class TestR9PromotionWarningsAsErrors(unittest.TestCase):
    ...
    def test_promote_all_single_w_code(self):
        ...
        failures = ["W571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=some"]
        result = _run_validate_dir_with_injected_failures(
            self.root, failures, {"SPECDEV_WARNINGS_AS_ERRORS": "1"}
        )
        self.assertEqual(len(result), 1)
        self.assertTrue(result[0].startswith("E571"))
        self.assertNotIn("W571", result[0])

    def test_promote_all_multiple_distinct_w_codes(self):
        """Multiple distinct W-codes are all promoted."""
        failures = [
            "W571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=some",
            "W593 VAGUE_LANGUAGE_FREE_TEXT spec/05.json ref=many",
            "W580 SUBSTEP_DRIFT spec/09.json ref=milestone-1",
        ]
        result = _run_validate_dir_with_injected_failures(
            self.root, failures, {"SPECDEV_WARNINGS_AS_ERRORS": "1"}
        )
        ...
        for line in result:
            self.assertFalse(line.startswith("W"), f"W-code not promoted: {line}")
            self.assertTrue(line.startswith("E"), f"Expected E-code: {line}")

    def test_promote_all_covers_every_promotable_pair(self):
        """Every W-code in PROMOTABLE_PAIRS is promoted to its E-code."""
        failures = [
            f"{w_code} TEST_LABEL spec/test.json ref=x" for w_code in PROMOTABLE_PAIRS
        ]
        result = _run_validate_dir_with_injected_failures(
            self.root, failures, {"SPECDEV_WARNINGS_AS_ERRORS": "1"}
        )
        self.assertEqual(len(result), len(PROMOTABLE_PAIRS))
        result_codes = {line.split()[0] for line in result}
    ...
    def test_promote_all_does_not_promote_non_promotable_codes(self):
        ...
        failures = ["W110 DEPRECATED_CANONICAL_USED spec/01.json ref=old-id"]
        result = _run_validate_dir_with_injected_failures(
            self.root, failures, {"SPECDEV_WARNINGS_AS_ERRORS": "1"}
        )
        self.assertEqual(len(result), 1)
        self.assertTrue(result[0].startswith("W110"))

    def test_promote_all_leaves_existing_e_codes_untouched(self):
        """E-codes already present are not double-promoted or corrupted."""
        failures = [
            "E571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=existing-error",
            "W571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=warn-to-promote",
        ]
        result = _run_validate_dir_with_injected_failures(
            self.root, failures, {"SPECDEV_WARNINGS_AS_ERRORS": "1"}
        )
        ...
        for line in result:
            self.assertTrue(line.startswith("E571"))
...
class TestR9PromotionSelectiveCodes(unittest.TestCase):
    ...
    def test_promote_single_code(self):
        ...
        codes = [line.split()[0] for line in result]
    ...
    def test_non_promotable_code_is_ignored(self):
        ...
        failures = [
            "W110 DEPRECATED_CANONICAL_USED spec/01.json ref=old-id",
        ]
        result = _run_validate_dir_with_injected_failures(
    ...
    def test_empty_promote_codes_no_promotion(self):
        ...
        self.assertEqual(len(result), 1)
        self.assertTrue(result[0].startswith("W571"))
...
class TestR9PromotionNoEnvVar(unittest.TestCase):
    ...
    def test_no_env_var_w_codes_remain(self):
        ...
        self.assertEqual(len(result), 2)
        for line in result:
            self.assertTrue(line.startswith("W"), f"Expected W-code: {line}")
    ...
    def test_no_env_var_keeps_w_when_no_matching_e(self):
        ...
        failures = [
            "W571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=some",
            "E593 VAGUE_LANGUAGE_FREE_TEXT spec/05.json ref=different-message",
        ]
        result = _run_validate_dir_with_injected_failures(self.root, failures)
    ...
    def test_no_env_var_dedup_multiple_pairs(self):
        ...
        failures = [
            "E571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=x",
            "W571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=x",
            "E580 SUBSTEP_DRIFT spec/09.json ref=y",
            "W580 SUBSTEP_DRIFT spec/09.json ref=y",
        ]
        result = _run_validate_dir_with_injected_failures(self.root, failures)
        self.assertEqual(len(result), 2)
        codes = [line.split()[0] for line in result]
    ...
    def test_exact_duplicate_failures_are_deduped(self):
        """Exact duplicate lines (same code, same message) are deduped via dict.fromkeys."""
        failures = [
            "W571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=some",
            "W571 ASSUMPTION_VAGUE_QUANTIFIER spec/04.json ref=some",
        ]
...
class TestR9PromotablePairsIntegrity(unittest.TestCase):
    ...
    def test_promotable_pairs_count(self):
        """PROMOTABLE_PAIRS has the expected 18 entries."""
        self.assertEqual(len(PROMOTABLE_PAIRS), 18)
...
class TestR9ExtractionIntentPipelineIntegration(unittest.TestCase):
    ...
    def test_extraction_intent_errors_surface_through_validate_dir(self):
        ...
        intent_errors = [
            "E597 EXTRACTION_INTENT_UPSTREAM_GAP step=05 missing_upstream=04",
            "W597 EXTRACTION_INTENT_VAGUE step=06 text='relevant stuff'",
        ]
