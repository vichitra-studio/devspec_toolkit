...
class TestDetectSpecRoot:
    def test_explicit_spec_dir(self, tmp_path):
        spec = tmp_path / "my_spec"
        result = _detect_spec_root(tmp_path, str(spec))
        assert result == spec

    def test_default_spec_dir(self, tmp_path):
        result = _detect_spec_root(tmp_path)
        assert result == tmp_path / "spec"
