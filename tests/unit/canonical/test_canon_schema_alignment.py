...
class CanonSchemaAlignmentTests(unittest.TestCase):
    ...
    def test_canon_enum_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            e554 = [e for e in errors if e.code == "E554"]
            self.assertEqual(len(e554), 1)
            self.assertIn("d", e554[0].render())
    ...
    def test_schema_enum_extra(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            e551 = [e for e in errors if e.code == "E551"]
            self.assertEqual(len(e551), 1)
            self.assertIn("c", e551[0].render())
    ...
    def test_missing_paired_schema(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            e552 = [e for e in errors if e.code == "E552"]
            self.assertEqual(len(e552), 1)
            self.assertIn("nonexistent.schema.json", e552[0].render())
    ...
    def test_missing_enum_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            e553 = [e for e in errors if e.code == "E553"]
            self.assertEqual(len(e553), 1)
            self.assertIn("test.schema.json", e553[0].render())
    ...
    def test_discovery_scan_warns(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("test_kind", w552[0].render())
