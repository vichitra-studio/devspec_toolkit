...
class TestStrictMode:
    ...
    def test_strict_mode_true_fails_on_mismatch(self, mismatched_layout):
        ...
        assert any("E520" in e for e in render_errors(errs))
    ...
    def test_strict_mode_false_warns_on_mismatch(self, mismatched_layout):
        ...
        # Should warn but not produce E520 for the mismatch itself
        has_warning = any("scope warning" in e for e in render_errors(errs))
        has_mismatch_error = any("E520" in e and "project_root_mismatch" in e for e in render_errors(errs))
        assert has_warning
        assert not has_mismatch_error
