...
_extract_free_text_tokens, tokenize_free_text, and edge cases around threshold
...
class TestTokenize(unittest.TestCase):
    """Tests for the tokenize_free_text helper."""

    def test_extracts_4plus_char_words(self):
        tokens = tokenize_free_text("The user authentication module handles login flow")
    ...
    def test_excludes_short_words(self):
        tokens = tokenize_free_text("a is of to in the")
        self.assertEqual(tokens, set())

    def test_excludes_3char_words(self):
        tokens = tokenize_free_text("use the api for fun")
        # "use", "the", "api", "for", "fun" are all 3 chars or less
        self.assertEqual(tokens, set())

    def test_excludes_stopwords(self):
        tokens = tokenize_free_text("this should have been true false null schema json")
        # all of these are in _DERIVATION_STOPWORDS
        self.assertEqual(tokens, set())

    def test_case_insensitive(self):
        tokens = tokenize_free_text("Authentication LOGIN")
        self.assertIn("authentication", tokens)
        self.assertIn("login", tokens)

    def test_hyphenated_and_underscored_tokens(self):
        tokens = tokenize_free_text("user-login session_token")
        self.assertIn("user-login", tokens)
        self.assertIn("session_token", tokens)

    def test_mixed_alphanum(self):
        tokens = tokenize_free_text("oauth2 error404")
    ...
    def test_tokens_starting_with_digit_excluded(self):
        ...
        tokens = tokenize_free_text("123invalid 4ever")
...
class TestCheckContentDerivation(unittest.TestCase):
    ...
    def test_sufficient_overlap_produces_no_w594(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            w594_errors = [e for e in errs if "W594" in e.render()]
    ...
    def test_low_overlap_emits_w594(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("CONTENT_DERIVATION_LOW_OVERLAP", w594_errors[0].render())
            self.assertIn("overlap=0", w594_errors[0].render())
    ...
    def test_missing_upstream_file_emits_w590(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            w590_errors = [e for e in errs if "W590" in e.render()]
            self.assertEqual(len(w590_errors), 1)
            self.assertIn("CROSS_STEP_UPSTREAM_MISSING", w590_errors[0].render())
            self.assertIn("00", w590_errors[0].render())
    ...
    def test_missing_upstream_with_other_upstream_present(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            w590_errors = [e for e in errs if "W590" in e.render()]
            self.assertEqual(len(w590_errors), 1)
            self.assertIn("01", w590_errors[0].render())
    ...
    def test_threshold_is_configurable(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            w594_pass = [e for e in errs_pass if "W594" in e.render()]
            ...
            w594_fail = [e for e in errs_fail if "W594" in e.render()]
            self.assertEqual(len(w594_fail), 1)
            self.assertIn("overlap=3", w594_fail[0].render())
            self.assertIn("threshold=4", w594_fail[0].render())
    ...
    def test_step_with_suffix_letter_recognized(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            w594_errors = [e for e in errs if "W594" in e.render()]
            self.assertEqual(len(w594_errors), 1)
            self.assertIn("02a_glossary.json", w594_errors[0].render())
