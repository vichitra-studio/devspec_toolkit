...
class TestTraceabilityClosure(unittest.TestCase):
    ...
    def test_charter_goal_without_capability_detected(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            errs = check_traceability_closure(d)
            self.assertTrue(
                any("charter_goal_without_capability" in e and "goal-perf" in e for e in errs),
                f"Expected E560 for goal-perf. Got: {errs}"
            )
            # goal-auth should NOT appear as a gap
            self.assertFalse(
                any("charter_goal_without_capability" in e and "goal-auth" in e for e in errs)
            )
    ...
    def test_full_chain_with_charter_valid(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            errs = check_traceability_closure(d)
            charter_errs = [e for e in errs if "charter_goal_without_capability" in e]
    ...
    def test_missing_spec_file_graceful(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            w570 = [e for e in errs if "W570" in e]
            hard_errors = [e for e in errs if not e.startswith("W")]
