...
- **`specdev matrix --out` default** (telemetry-derived ergonomics fix): the matrix subcommand's `--out` flag now defaults to `<spec_dir>/extras/trace_matrix.json` (resolved via `core.constants.resolve_extras_path`) instead of `-` (stdout). `specdev matrix spec` is now equivalent to the previously-required `specdev matrix spec --out spec/extras/trace_matrix.json`. Explicit `--out -` (stdout) and `--out <path>` continue to work unchanged. When the default is used, the printed path is the resolved **absolute** path returned by `resolve_extras_path`; when `--out` is supplied, the printed string is unchanged (still the user-supplied value). Callers that captured `OUT=$(specdev matrix spec ...)` expecting a relative path and need that exact format should pass `--out spec/extras/trace_matrix.json` explicitly. `prompts/prompt_14_roadmap.md` updated to read coverage from the written file via `specdev json read spec/extras/trace_matrix.json '.coverage'`.

- **`specdev json …` flag tolerance** (telemetry-derived ergonomics fix): most `json` subcommands (`read`, `read-multi`, `keys`, `structure`, `schema`, `patch`, `insert`, `delete`) now silently strip `--spec-root` and `--git-root` from their argument list before dispatch. These flags are accepted by validation/governance commands (`spec-check`, `validate`, `matrix`, `governance-check`) but were rejected by these `json` subcommands with `unrecognized arguments`; agents over-applying the host-wide "always pass three flags" guidance hit this in volume. Behaviour change: `specdev json read … --spec-root ./spec --git-root .` now succeeds instead of failing. Look-ahead protects subsequent flags from being consumed as the missing value when the user wrote `--spec-root --repo-root foo`. **Exception**: `specdev json resolve-pointers` is excluded from the strip — it legitimately accepts `--git-root` (anchors relative file-path resolution) and emits a deprecation warning for `--spec-root`. Hard-error behaviour for other unknown arguments is preserved; only these two flags are dropped, and only on the read/edit subcommands. `--repo-root` continues to be accepted normally.
